...
/// PostgreSQL-backed durable production store.
#[derive(Debug, Clone)]
...
impl PostgresStore {
    /// Creates a `PostgreSQL` store from a pool.
    #[must_use]
    ...
    pub async fn setup_schema(&self) -> Result<(), StoreError> {
        for statement in include_str!("../../migrations/postgres/001_budget_warden.sql")
            .split(';')
            .map(str::trim)
...
impl BudgetStore for PostgresStore {
    async fn reserve_if_allowed(
        ...
        query::<Postgres>(
            "INSERT INTO budget_warden_counters \
             (store_key, window_start, window_end, committed, reserved) VALUES ($1, $2, $3, 0, 0) \
             ON CONFLICT (store_key, window_start, window_end) DO NOTHING",
        )
        .bind(&store_key)
        ...
        let update = query::<Postgres>(
            "UPDATE budget_warden_counters SET reserved = reserved + $1, updated_at = now() \
             WHERE store_key = $2 AND window_start = $3 AND window_end = $4 \
             AND committed + reserved + $5 <= $6",
        )
        .bind(amount)
        .bind(&store_key)
        .bind(codec::window_start(request.window))
        ...
        let row = query::<Postgres>(
            "INSERT INTO budget_warden_reservations \
             ...
             VALUES ($1, $2, $3, $4, 'active', $5, $6) RETURNING reservation_id",
        )
        .bind(&store_key)
        .bind(codec::window_start(request.window))
        .bind(codec::window_end(request.window))
...
async fn existing_idempotent_reservation(
    ...
    let row = query::<Postgres>(
        "SELECT reservation_id, expires_at FROM budget_warden_reservations \
         WHERE store_key = $1 AND window_start = $2 AND window_end = $3 AND idempotency_key = $4 \
...
async fn cleanup_expired(tx: &mut PgTransaction<'_>, now: DateTime<Utc>) -> Result<(), StoreError> {
    let expired = query::<Postgres>(
        "SELECT reservation_id, store_key, window_start, window_end, amount \
         FROM budget_warden_reservations WHERE status = 'active' AND expires_at <= $1 FOR UPDATE",
    )
    .bind(now)
    .fetch_all(&mut **tx)
    .await
    ...
    for row in expired {
        ...
        query::<Postgres>(
            "UPDATE budget_warden_counters SET reserved = GREATEST(reserved - $1, 0), updated_at = now() \
             WHERE store_key = $2 AND window_start = $3 AND window_end = $4",
        )
        .bind(amount)
        ...
        .map_err(sql_error)?;
        query::<Postgres>(
            "UPDATE budget_warden_reservations SET status = 'expired' WHERE reservation_id = $1",
        )
        .bind(reservation_id)
        .execute(&mut **tx)
        .await
...
async fn load_usage(
    ...
    let row = query::<Postgres>(
        "SELECT committed, reserved FROM budget_warden_counters \
         WHERE store_key = $1 AND window_start = $2 AND window_end = $3",
    )
    .bind(store_key)
    .bind(codec::window_start(window))
    .bind(codec::window_end(window))
...
async fn finish_reservation(
    ...
    let row = query::<Postgres>(
        "SELECT store_key, window_start, window_end, amount, status, expires_at \
         FROM budget_warden_reservations WHERE reservation_id = $1 FOR UPDATE",
    )
    .bind(id)
    .fetch_optional(&mut *tx)
    .await
    ...
    match status.as_str() {
        ...
        "active" => {
            ...
            query::<Postgres>(
                "UPDATE budget_warden_counters SET reserved = GREATEST(reserved - $1, 0), \
                 committed = committed + $2, updated_at = now() \
                 WHERE store_key = $3 AND window_start = $4 AND window_end = $5",
            )
            .bind(amount)
            ...
            .map_err(sql_error)?;
            query::<Postgres>(
                "UPDATE budget_warden_reservations SET status = $1 WHERE reservation_id = $2",
            )
            .bind(status)
            .bind(id)
            .execute(&mut *tx)
